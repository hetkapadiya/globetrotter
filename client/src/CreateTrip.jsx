...
export default function CreateTrip({ onNavigate }) {
  ...
  const moveCity = (index, direction) => {
    ...
    if (newIndex < 0 || newIndex >= newCities.length) {
      return;
    }

    [newCities[index], newCities[newIndex]] = [
      newCities[newIndex],
      newCities[index],
    ];
  ...
  const createTrip = async () => {
    ...
    try {
      ...
      const stops = selectedCities.map((city, index) => {
        const totalDays = days || selectedCities.length;

        const tripStart = new Date(startDate);

        const stopStart = new Date(tripStart);

        stopStart.setDate(stopStart.getDate() + index);

        const stopEnd = new Date(stopStart);

        const remainingDays = totalDays - index - 1;

        if (remainingDays > 0) {
          stopEnd.setDate(stopEnd.getDate() + 1);
        } else {
          stopEnd.setTime(new Date(endDate).getTime());
        }
      ...
      setSuccess(`Trip "${result.data.name}" created successfully!`);
    ...
    <div className="create-trip-page">
      ...
      <div className="create-trip-header">
        <div>
          ...
          <p>
            Build your itinerary, choose your destinations and set your travel
            budget.
          </p>
        </div>
      </div>
        ...
        <div className="create-trip-main">
          <section className="form-card">
            <div className="form-card-heading">
              ...
              <div>
                <h2>Trip details</h2>
                <p>Give your adventure a name and choose your dates.</p>
              </div>
            </div>
          ...
          <section className="form-card">
            <div className="form-card-heading">
              ...
              <div>
                ...
                <p>Add cities in the order you want to visit them.</p>
              </div>
            </div>
            ...
            <div className="city-picker">
              {loadingCities ? (
                <div className="picker-message">Finding destinations...</div>
              ) : cities.length === 0 ? (
                <div className="picker-message">No destinations found.</div>
              ) : (
                cities.map((city) => {
        ...
        <aside className="create-trip-sidebar">
          <section className="summary-card">
            ...
            <div className="summary-budget">
              ...
              <strong>₹{Number(budget || 0).toLocaleString("en-IN")}</strong>
            </div>
